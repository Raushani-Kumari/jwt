...
import { clearToken, getRefreshTokenWithExpiry, getTokenWithExpiry, saveTokenWithExpiry } from "../utils/tokenUtil";

const api_path = 'http://localhost:8080/api';

export const register = async ({ username, email, password }) => {
    try {
        const response = await axios.post(`${api_path}/register`, { username, email, password });
...
export const apiRequest = async (endpoint, method = 'GET', data = null) => {
    ...
    try {
        ...
        if (error.response?.status === 401 && refreshToken) {
            ...
            try {
                const refreshResponse = await axios.post(`${api_path}/refresh-token`, { refreshToken });
                const { token: newAccessToken, refreshToken: newRefreshToken, tokenExpiry, refreshTokenExpiry } = refreshResponse.data;

                console.log('New access token:', newAccessToken);
                console.log('New refresh token:', newRefreshToken);

                saveTokenWithExpiry(newAccessToken, newRefreshToken, tokenExpiry, refreshTokenExpiry);
